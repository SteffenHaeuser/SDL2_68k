...
void OS3_PumpEvents(_THIS)
{
    ...
    for (sdlwin = _this->windows; sdlwin; sdlwin = sdlwin->next) {
        ...
        while ((imsg = (struct IntuiMessage *)GetMsg(intuiwin->UserPort)) != NULL) {
            ...
            case IDCMP_NEWSIZE:
            {
                int neww = (int)intuiwin->Width - (int)intuiwin->BorderLeft - (int)intuiwin->BorderRight;
                int newh = (int)intuiwin->Height - (int)intuiwin->BorderTop - (int)intuiwin->BorderBottom;
                if (neww <= 0) neww = (int)intuiwin->GZZWidth;
                if (newh <= 0) newh = (int)intuiwin->GZZHeight;
#if defined(SDL_VIDEO_OPENGL)
                if (data->is_opengl) {
                    OS3_GL_ResizeWindow(_this, sdlwin, neww, newh);
                }
#endif
                SDL_SendWindowEvent(sdlwin, SDL_WINDOWEVENT_RESIZED, neww, newh);
                break;
            }
