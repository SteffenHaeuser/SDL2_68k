...
/*
 * Some AmigaOS SDL2 configurations define these old integer aliases as
 * preprocessor macros in SDL_config.h. stb_vorbis declares its own typedefs
 * with the same names, so remove the macros before including stb_vorbis.h.
 */
#ifdef uint8
#undef uint8
#endif
