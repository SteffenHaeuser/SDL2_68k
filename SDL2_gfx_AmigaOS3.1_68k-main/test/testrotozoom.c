	...
	for (framecount=-360; framecount<framemax && !done; framecount += frameinc) {
		...
		if (flip) {
			...
			if (((framecount % 120)==0) || (delay>0)) {
				SDL_Log("  Frame: %i   Rotate: angle=%d  Zoom: x=%d y=%d\n",framecount,(unsigned int)angle,(unsigned int)zoomfx,(unsigned int)zoomfy);
			}
			...
			angle=framecount*rotate;
			if ((framecount % 120)==0) {
				SDL_Log("  Frame: %i   Rotate: angle=%d  Zoom: f=%d \n",framecount,(unsigned int)angle,(unsigned int)zoomf);
			}
...
void ZoomPicture (SDL_Surface *picture, int smooth) 
{
	...
	for (framecount=360; framecount<framemax && !done; framecount += frameinc) {
		...
		if ((framecount % 120)==0) {
			SDL_Log("  Frame: %i   Zoom: x=%d y=%d\n",framecount,(unsigned int)zoomxf,(unsigned int)zoomyf);
		}
