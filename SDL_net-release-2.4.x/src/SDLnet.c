...
static int amiga_socket_errno = 0;
#endif

#if !defined(__USE_W32_SOCKETS) && !defined(SDLNET_AMIGAOS)
#include <signal.h>
#endif
...
{
    #if defined(SDLNET_AMIGAOS)
    return amiga_socket_errno;
...
int  SDLNet_Init(void)
{
    if ( !SDLNet_started ) {
        ...
        if (SocketBase == NULL) {
            SDLNet_SetError("Couldn't open bsdsocket.library v4");
            return -1;
        }
        amiga_socket_errno = 0;
        SetErrnoPtr(&amiga_socket_errno, sizeof(amiga_socket_errno));
...
#ifdef SDLNET_AMIGAOS
    (void)in;
    return (const char *)Inet_NtoA(ip->host);
#else
    in.s_addr = ip->host;
    return inet_ntoa(in);
#endif
